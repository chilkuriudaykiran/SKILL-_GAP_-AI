...
# Import required packages with friendly fallbacks that give actionable messages when missing
missing_packages = []
try:
...
if missing_packages:
    ...
    # If Streamlit is available, show a friendly message inside the app; otherwise print and exit.
    try:
        # If st is available (it may be missing and listed in missing_packages), use it to show the error
        import sys
        if 'streamlit' in missing_packages:
            print(message)
        else:
            st.error(message)
            st.stop()
...
def load_model():
    try:
        ...
        # Show an actionable error to the user inside Streamlit and stop the app gracefully
        st.error("Error loading SentenceTransformer model: " + str(e))
        st.stop()
